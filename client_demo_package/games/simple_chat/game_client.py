"""
Simple Chat Game - Client
A basic chat client with CLI interface
"""
import socket
import threading
import argparse
...
class ChatClient:
    ...
    def run(self):
        """Run the client"""
        print("="*60)
        print(f"   Simple Chat - Player: {self.username}")
        print("="*60)
        ...
        try:
            while self.running:
                ...
                # Print own message locally (won't receive echo from server)
                print(f"\n[{self.username}] {text}")
